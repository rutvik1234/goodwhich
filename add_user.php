...
if (isset($_POST['add_user'])) {
  ...
  $req_fields = array('name', 'phone', 'email', 'username', 'password', 'level');
  ...
  if (empty($errors)) {
    ...
    $query .= "name,phone,email,username,password,user_level,status";
    $query .= ") VALUES (";
    $query .= " '{$name}', '{$phone}', '{$email}','{$username}', '{$password}' , '{$user_level}','1'";
    $query .= ")";
    if ($db->query($query)) {
      //sucess
